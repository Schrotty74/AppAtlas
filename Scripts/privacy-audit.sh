...
if grep -I -n -E '/Users/[^/]+|/Volumes/[^/]+|serialNumber[[:space:]]*[:=][[:space:]]*"[^"$]+' \
    "${content_files[@]}" >/tmp/appatlas-privacy-audit.txt 2>/dev/null; then
    cat /tmp/appatlas-privacy-audit.txt >&2
    echo "Datenschutzaudit fehlgeschlagen: persönlicher Pfad oder Geheimnis gefunden." >&2
    exit 1
fi

history_findings="/tmp/appatlas-privacy-history-audit.txt"
